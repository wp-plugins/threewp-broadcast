...
class input
{
	use \plainview\sdk\html\element;
	...
	**/
	...
	{
		$random = \plainview\sdk\base::uuid();
	...
	**/
	public function display_input()
	{
		...
		if ( $input->form()->post_is_set() )
		{
			...
			if ( $value != '' )
			{
				$value = \plainview\sdk\form2\form::unfilter_text( $value );
				$input->value( $value );
			}
			else
	...
	**/
	public function make_id()
	{
		if ( $this->has_attribute( 'id' ) )
			return $this->get_attribute( 'id' );
		$id = $this->make_name();
		$id = \plainview\sdk\base::strtolower( $id );
	...
	**/
	...
	{
		$title = \plainview\sdk\form2\form::filter_text( $title );
